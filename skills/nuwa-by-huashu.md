...
---
name: nuwa-by-huashu
description: 女娲造人。输入人名/主题/甚至是模糊需求，自动深度调研→思维框架提炼→生成可运行的人物Skill。两种入口：(1)明确人名→直接蒸馏 (2)模糊需求→诊断推荐→再蒸馏。触发词：造skill、蒸馏XX、女娲造人、XX的思维方式、做个XX视角、更新XX的skill。模糊需求触发："我想提升决策质量"、"有没有一种思维方式能帮我..."、"我需要一个思维顾问"。
---
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查 `/example` 目录）
...
**来源A：本地已有Skill**
扫描 `/skills/nuwa-huashu/example/` 目录，读取每个SKILL.md的description，匹配用户需求。
已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
**收到确认后立即执行**，在调研之前完成，在下方指定路径下创建skill目录，用以存放后续产物

```
/mnt/agents/output/[person-name]/
├── SKILL.md                          # 最终产物
...
**若当前环境具备subagent能力或处在Swarm模式下，启动subagent进行信息搜集：**
...
**若当前环境不具备subagent能力，启动则依赖主Agent能力尽可能多地进行信息搜集，参考上面6个subagent的搜集任务**
...
Phase 1启动前，**主动扫描 `/app/.agents/skills/`和 `/app/.user/skills/` 目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `/mnt/agents/output/[person-name]-perspective/SKILL.md`
将/mnt/agents/output/[person-name]-perspective 文件夹进行打包，生成 /mnt/agents/output/[person-name].skill 文件
在你回答的末尾，输出 <KIMI_REF type="file" path="sandbox:///mnt/agents/output/[person-name].skill" />，以保证用户可在对话页面中看到生成的skill产物。
...
*Skill source: /app/.agents/skills/nuwa-by-huashu/SKILL.md*
