...
When creating additional resource directories, **do not use bash brace expansion** (e.g. `mkdir -p skill/{scripts,references}`) — the shell environment uses `/bin/sh` which does not support it and will silently create a literal directory named `{scripts,references}` instead. Create directories one at a time or use Python:
...
# Also correct
import os
os.makedirs('/path/to/skill/scripts', exist_ok=True)
os.makedirs('/path/to/skill/references', exist_ok=True)
```
...
---
*Skill path: `/app/.agents/skills/skill-creator/SKILL.md`*
